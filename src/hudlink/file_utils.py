...
def clean_up_eligibility_df(df):
    """
    Clean up the eligibility DataFrame by renaming columns, dropping unnecessary ones,
    and reordering for better readability.
    
    Parameters:
        df (pd.DataFrame): The eligibility DataFrame to clean up
        
    Returns:
        pd.DataFrame: Cleaned and reorganized DataFrame
    """
    df = df.copy()
    ...
    rename_dict = {
        ...
        # Example:
        'ACTUAL_HH_INCOME': 'HH_INCOME',
        'REALHHWT': 'HHWT_stable',
        'Allocated_HHWT': 'HHWT_fam_adjusted', 
        'HHWT': 'HHWT_unmodified', 
        'State_Alpha%': 'State',
        'ADJUSTED_FAMSIZE': 'FAMSIZE_adjusted',
        'elig_2adults': '2adults',
        'elig_1adult': '1adult',
        'elig_female_head': 'female_head',
        'elig_female_head_child': 'female_head_child',
        'elig_male_head': 'male_head',
        'elig_male_head_child': 'male_head_child',
        'elig_age62plus': 'age62plus',
        'elig_age75plus': 'age75plus',
        'elig_minority': 'minority',
        'elig_white_nonhsp': 'white_nonhsp',
        'elig_black_nonhsp': 'black_nonhsp',
        'elig_native_american_nonhsp': 'native_american_nonhsp',
        'elig_asian_nonhsp': 'asian_nonhsp',
        'elig_mixed_nonhsp': 'mixed_nonhsp',
        'elig_otherrace': 'otherrace',
        'elig_hispanic': 'hispanic',
        'elig_noncitizen': 'noncitizen',
        'elig_owner': 'owner',
        'elig_renter': 'renter',
        'elig_mortgage_paid': 'mortgage_paid',
        'elig_disab_hearing_vision': 'disab_hearing_vision',
        'elig_disab_ambulatory': 'disab_ambulatory',
        'elig_disab_cognitative': 'disab_cognitative',
        'elig_disab_independent_living': 'disab_independent_living',
        'elig_veteran': 'veteran',
        'elig_hs_complete': 'hs_complete',
        'elig_bachelor_complete': 'bachelor_complete',
        'elig_grad_school': 'grad_school',
        'elig_employed': 'employed',
        'elig_disab_any': 'disab_any',
        'CBSERIAL': 'HOUSEHOLD_NUMBER/CBSERIAL'
    }
    
    # Define columns to drop
    columns_to_drop = [

        'County_Name_state_abbr',
        'County_Name_Alt',
        'County',
        'County code'
    ]
    
    # Define desired column order (put most important columns first)
    ...
    # Reorder columns (keeping any columns not specified at the end)
    if desired_column_order:
        # Get columns that exist in both the DataFrame and desired order
...
def tidy_summary_df(summary_df, state):
    """
    Enhanced tidy function for summary DataFrame with additional cleanup options.
    ...
    Returns:
        pd.DataFrame: Cleaned summary DataFrame
    """
    ...
    # Example: Drop additional columns if needed
    additional_drops = [
        'imputed_from_prev', 
        'fedhse', 
        'cbsa', 
    ...
    # Example: Reorder columns for summary table
    summary_column_order = [

        'County_Name',
    ]
    
    if summary_column_order:
        existing_ordered_cols = [col for col in summary_column_order if col in df.columns]
        remaining_cols = [col for col in df.columns if col not in existing_ordered_cols]
...
def add_fips_codes_to_df(df, state, fips_mapping=None):
    """
    Add FIPS codes to DataFrame based on County_Name and state.
    
    Parameters:
        df (pd.DataFrame): DataFrame with County_Name column
        state (str): State abbreviation (e.g., 'FL')
        fips_mapping (dict, optional): FIPS mapping dictionary. 
                                     If None, uses COUNTY_FIPS_MAPPING from module.
        
    Returns:
        pd.DataFrame: DataFrame with added FIPS_Code column
    """
    # Create a copy to avoid modifying original
    result_df = df.copy()
    
    # Use provided mapping or fall back to module-level mapping
    mapping = fips_mapping if fips_mapping is not None else COUNTY_FIPS_MAPPING
    
    # Add FIPS codes using vectorized lookup
    result_df['FIPS_Code'] = result_df['County_Name'].apply(
        lambda county: mapping.get((state.upper(), county))
    )
    
    # Log any missing FIPS codes
    missing_fips = result_df[result_df['FIPS_Code'].isna()]['County_Name'].unique()
    if len(missing_fips) > 0:
        logging.warning(f"Counties without FIPS codes in {state}: {list(missing_fips)}")
    else:
        logging.info(f"Successfully added FIPS codes for all counties in {state}")
...
COUNTY_FIPS_MAPPING = {
    ...
    ('AK', 'Petersburg Census Area'): '02280',
    ('AK', 'Prince of Wales-Hyder Census Area'): '02201',
    ('AK', 'Sitka Borough'): '02220',
    ('AK', 'Skagway Municipality'): '02232',
    ('AK', 'Southeast Fairbanks Census Area'): '02240',
    ('AK', 'Yakutat Borough'): '02282',
    ...
    ###################################################################
    ######################## Patch Below:#############################
    ('CT', 'Capitol Planning Region'):                     '09003',
    ('CT', 'Greater Bridgeport Planning Region'):          '09001',
    ('CT', 'Lower Connecticut River Valley Planning Region'):'09007',
    ('CT', 'Naugatuck Valley Planning Region'):             '09140',
    ('CT', 'Northeastern Connecticut Planning Region'):    '09013',     ####These are not the 'correct' PUMAS for the planning regionas
    ('CT', 'Northwest Hills Planning Region'):             '09005',     #### These are a patch until all other software can accomodate the new 
    ('CT', 'South Central Connecticut Planning Region'):   '09009',     #### FIPS codes
    ('CT', 'Southeastern Connecticut Planning Region'):    '09011',
    ('CT', 'Western Connecticut Planning Region'):         '09190',
    #################################################################
    ######################COrrect FIPS CODES BELOW:##################
    # ('CT', 'Capitol Planning Region'):                     '09110',    ### These can be uncommented once plotly and other viz software can 
    # ('CT', 'Greater Bridgeport Planning Region'):          '09120',    ### accomodate the new CT county equivalents
    ...
    ('VA', 'Alexandria city County'): '51510',
    ...
    ('VA', 'Charlottesville city County'): '51540',
    ('VA', 'Chesapeake city County'): '51550',
    ('VA', 'Chesterfield County'): '51041',
    ('VA', 'Clarke County'): '51043',
    ('VA', 'Clifton Forge city County'): '51560',
    ('VA', 'Colonial Heights city County'): '51570',
    ('VA', 'Covington city County'): '51580',
    ('VA', 'Craig County'): '51045',
    ('VA', 'Culpeper County'): '51047',
    ('VA', 'Cumberland County'): '51049',
    ('VA', 'Danville city County'): '51590',
    ('VA', 'Dickenson County'): '51051',
    ('VA', 'Dinwiddie County'): '51053',
    ('VA', 'Emporia city County'): '51595',
    ('VA', 'Essex County'): '51057',
    ('VA', 'Fairfax city County'): '51600',
    ('VA', 'Fairfax County'): '51059',
    ('VA', 'Falls Church city County'): '51610',
    ('VA', 'Fauquier County'): '51061',
    ('VA', 'Floyd County'): '51063',
    ('VA', 'Fluvanna County'): '51065',
    ('VA', 'Franklin city County'): '51620',
    ('VA', 'Franklin County'): '51067',
    ('VA', 'Frederick County'): '51069',
    ('VA', 'Fredericksburg city County'): '51630',
    ('VA', 'Galax city County'): '51640',
    ...
    ('VA', 'Lee County'): '51105',
    ('VA', 'Lexington city County'): '51678',
    ('VA', 'Loudoun County'): '51107',
    ('VA', 'Louisa County'): '51109',
    ('VA', 'Lunenburg County'): '51111',
    ('VA', 'Lynchburg city County'): '51680',
    ('VA', 'Madison County'): '51113',
    ('VA', 'Manassas city County'): '51683',
    ('VA', 'Manassas Park city County'): '51685',
    ('VA', 'Martinsville city County'): '51690',
    ...
    ('VA', 'Newport News city County'): '51700',
    ('VA', 'Norfolk city County'): '51710',
    ('VA', 'Northampton County'): '51131',
    ('VA', 'Northumberland County'): '51133',
    ('VA', 'Norton city County'): '51720',
    ('VA', 'Nottoway County'): '51135',
    ('VA', 'Orange County'): '51137',
    ('VA', 'Page County'): '51139',
    ('VA', 'Patrick County'): '51141',
    ('VA', 'Petersburg city County'): '51730',
    ('VA', 'Pittsylvania County'): '51143',
    ('VA', 'Poquoson city County'): '51735',
    ('VA', 'Portsmouth city County'): '51740',
